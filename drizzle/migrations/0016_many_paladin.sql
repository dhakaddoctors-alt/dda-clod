...
CREATE TABLE `__new_candidates` (
	...
	FOREIGN KEY (`election_id`) REFERENCES `elections`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`post_id`) REFERENCES `election_posts`(`id`) ON UPDATE no action ON DELETE no action,
...
--> statement-breakpoint
INSERT INTO `__new_candidates`("id", "election_id", "post_id", "profile_id", "manifesto", "poster_url", "proposer_id", "seconder_id", "proposer_status", "seconder_status", "status") SELECT "id", "election_id", "post_id", "profile_id", "manifesto", "poster_url", "proposer_id", "seconder_id", "proposer_status", "seconder_status", "status" FROM `candidates`;--> statement-breakpoint
DROP TABLE `candidates`;--> statement-breakpoint
ALTER TABLE `__new_candidates` RENAME TO `candidates`;--> statement-breakpoint
PRAGMA foreign_keys=ON;--> statement-breakpoint
ALTER TABLE `vote_tallies` ADD `post_id` text REFERENCES election_posts(id);--> statement-breakpoint
ALTER TABLE `voting_records` ADD `post_id` text REFERENCES election_posts(id);